...
import com.rideflag.app.kotlin.VisionProcessorBase
...
class FaceDetectorProcessor(context: Context, detectorOptions: FaceDetectorOptions?) :
  ...
  //  Log.v("Jitinn", "FaceDetectorProcessor: fcs int=0 fcs: "+fcs)
  var fcs: Int = 0

  override fun onSuccess(faces: List<Face>, graphicOverlay: GraphicOverlay) {
    for (face in faces) {
      graphicOverlay.add(FaceGraphic(graphicOverlay, face, faces.size.toString()))
      logExtrasForTesting(face)




      Log.v("Jitinn", "FaceDetectorProcessor: face size "+faces.size.toString())
      fcs = faces.size
      Log.v("Jitinn", "FaceDetectorProcessor: fcs value: "+fcs)

    }
  }

   fun fcsValue() : Int{
    Log.v("Jitinn", "FaceDetectorProcessor: fcsValue() "+fcs)
